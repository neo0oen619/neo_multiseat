...
# --- Visual helpers --------------------------------------------------
$script:Esc = [char]27  # for optional ANSI bold if console supports it
...
function Credit-Banner-Begin {
    Show-ImportantBanner -Text "Binaries & INI updates from your fork: github.com/neo0oen619/rdpwrap" -Fg Black -Bg Cyan
    Write-Host "Original project by Stas'M; autoupdate maintained by asmtron." -ForegroundColor DarkGray
}

function Show-PurplePulse {
    param([string]$Text)
    $esc = $script:Esc
    $rgb = @("38;2;160;0;200","38;2;185;0;215","38;2;205;0;230","38;2;185;0;215","38;2;160;0;200")
    try {
        foreach ($c in $rgb) {
            $line = "$esc[1m$esc[$c" + "m" + $Text + "$esc[0m"
            Write-Host $line
        }
    } catch {
        foreach ($cc in @('Magenta','DarkMagenta','Magenta')) { Write-Host $Text -ForegroundColor $cc }
    }
}
...
function Show-FinaleCredits {
    ...
    Write-Host ""
    Show-ImportantBanner -Text "Original work and autoupdate credit: https://github.com/asmtron/rdpwrap" -Fg White -Bg DarkBlue
    Show-PurplePulse "made with <3 by neo0oen"
    $script:CreditsShown = $true
}

# --- RDP file helper (username + timestamp) ---------------------------
function New-Seat2RdpFile {
    param([Parameter(Mandatory=$true)][string]$TargetUser)

    # Find primary IPv4 address (skip APIPA and loopback). Fallback to PC name.
    $ips = Get-NetIPAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue |
           Where-Object { $_.IPAddress -notlike '169.254.*' -and $_.IPAddress -ne '127.0.0.1' } |
           Select-Object -ExpandProperty IPAddress
    $primary = $null
    if ($ips) { $primary = ($ips | Select-Object -First 1) }
    if (-not $primary) { $primary = $env:COMPUTERNAME }

    # Minimal .rdp for compatibility
    $content = @"
...
# --- Download helpers (from your fork) --------------------------------
$ForkBase = 'https://raw.githubusercontent.com/neo0oen619/rdpwrap/master'
$DL = @{
  RDPWInst = "$ForkBase/bin/RDPWInst.exe"
  RDPConf  = "$ForkBase/bin/RDPConf.exe"
  RDPCheck = "$ForkBase/bin/RDPCheck.exe"
  AutoZip1 = "$ForkBase/autoupdate_v1.2.zip"
  AutoZip2 = "$ForkBase/autoupdate.zip"
}
...
function Ensure-ForkBinaries {
    $prog = "${env:ProgramFiles}\RDP Wrapper"
    if (-not (Test-Path $prog)) { New-Item -ItemType Directory -Path $prog -Force | Out-Null }

    $need = @(
        @{Name='RDPWInst.exe'; Url=$DL.RDPWInst},
        @{Name='RDPConf.exe' ; Url=$DL.RDPConf },
        @{Name='RDPCheck.exe'; Url=$DL.RDPCheck}
    )

    foreach ($f in $need) {
        $dst = Join-Path $prog $f.Name
        if (-not (Test-Path $dst)) {
            if (-not (Get-WebFile -Uri $f.Url -OutFile $dst)) {
                throw "Could not download required file from your fork: $($f.Url)"
            }
        }
    }

    $autoDst = Join-Path $prog 'autoupdate.zip'
    if (-not (Test-Path (Join-Path $prog 'autoupdate.bat'))) {
        if (-not (Get-WebFile -Uri $DL.AutoZip1 -OutFile $autoDst)) {
            if (-not (Get-WebFile -Uri $DL.AutoZip2 -OutFile $autoDst)) {
                throw "Could not download autoupdate.zip from your fork."
            }
        }
        try {
            Add-Type -AssemblyName System.IO.Compression.FileSystem
            if (Test-Path (Join-Path $prog 'autoupdate.bat')) { Remove-Item (Join-Path $prog 'autoupdate.bat') -Force -ErrorAction SilentlyContinue }
            [IO.Compression.ZipFile]::ExtractToDirectory($autoDst, $prog, $true)
        } catch {
            throw "Failed to extract autoupdate.zip: $($_.Exception.Message)"
        } finally {
            Remove-Item $autoDst -Force -ErrorAction SilentlyContinue
        }
    }
}
...
function Get-RealLocalUsers {
    ...
    try {
        Get-LocalUser | Where-Object {
            $_.Enabled -and ($builtIns -notcontains $_.Name)
        } | Sort-Object Name
    } catch {
...
function Ensure-User {
    ...
    if ($users.Count) {
        $i = 1
        foreach ($u in $users) { Write-Host ("[{0}] {1}" -f $i, $u.Name) ; $i++ }
    } else {
    ...
    do {
        ...
        if ($sel -match '^(?i)N$') {
            do {
                $newName = Read-Host "Enter new username (must not be empty)"
            } until (-not [string]::IsNullOrWhiteSpace($newName))
...
function Remove-SeatUser {
    ...
    do {
        ...
        elseif ($sel -match '^\d+$' -and [int]$sel -ge 1 -and [int]$sel -le $users.Count) {
            $chosen = $users[[int]$sel - 1].Name
            if ($chosen -eq $env:USERNAME) {
                Write-Warning "Refusing to delete the currently logged-in user ($chosen)."
                return
            }
...
function Open-RDP-Folder {
    $prog = "${env:ProgramFiles}\RDP Wrapper"
    if (Test-Path $prog) {
        Start-Process explorer.exe $prog
    } else {
        Write-Warning "RDP Wrapper folder not found at: $prog"
    }
}
...
function Enable-RDP-And-Firewall {
    ...
    for ($n=1; $n -le $max; $n++) {
        try {
            ...
            Write-Warning ("Attempt {0} of {1}: Failed to start RDP services. Retrying in 2 seconds... {2}" -f $n, $max, $_.Exception.Message)
            Start-Sleep -Seconds 2
        }
    }
    return $false
...
function Install-Or-Update-RDPWrapper {
    Write-Host "`n=== Deploying RDP Wrapper from your fork ==="
    Credit-Banner-Begin
    $prog = "${env:ProgramFiles}\RDP Wrapper"
    Ensure-ForkBinaries
    ...
    & $inst -i | Write-Host
    & $inst -w | Write-Host   # refresh INI the supported way
    if (Test-Path $auto) {
        Write-Host "`n=== Running RDP Wrapper Autoupdate (from your fork) ==="
...
function Fix-RDP-Service {
    ...
    try {
        ...
        if (Test-Path $uninst) {
            Start-Process -FilePath $uninst -Verb RunAs -Wait
        }
...
function Seat2-InstallFlow {
    ...
    # Always create the timestamped RDP file for convenience
